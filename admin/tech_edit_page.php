...
<?php
    ...
    $edit_id = @$_GET['tech_edit'];
    $query = "select * from tech_posts where id= '$edit_id'";
    ...
    while( $row = mysql_fetch_array($run))
	{
            ...
            $socail_link = $row['tech_socail_link'];
            $image = $row['tech_image'];
            $content = $row['tech_content'];            
?>

    <form action="tech_edit_page.php?tech_edit_form=<?php echo $edit_id1 ?>" method='post' enctype="multipart/form-data">
    ...
    <?php 
        if(isset($_POST['update']))
        {
            ...
            $update_query = " update tech_posts set tech_title='$post_title',tech_date='$post_date',tech_author='$post_author',tech_socail_link='$post_socail_link',tech_image='$post_image',tech_content='$post_content' 
                        where id ='$update_id'";
            if (mysql_query($update_query)) 
            {
                echo "<script>alert('Tech Post has been updated')</script>";
                echo "<script>window.open('index.php?tech_post=Tech Post has been updated...','_self')</script>";
            } 
